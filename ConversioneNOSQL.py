...
def filter_lineups(lineups_data):
    """
...
for x in range(1960, 2025, 4):
    try:
        ...
        for _, row in df.iterrows():
            ...
            for col in df.columns:
                ...
                if col in columns_to_exclude:
                    continue
                parsed_value = parse_field(row[col])
                if col == "goals":
                    doc[col] = filter_goals(parsed_value)
                elif col == "home_lineups" or col == "away_lineups":
                    doc[col] = filter_lineups(parsed_value)
        ...
        with open(f"DatasetNoSQL/{x}.json", "w", encoding="utf-8") as f:
            json.dump(documents, f, ensure_ascii=False, indent=2)

        print(f"Conversione completata. File salvato come DatasetNoSQL/{x}.json.")
